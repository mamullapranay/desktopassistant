...
def wishme():
    ...
    if hour>=0 and hour<12 :
        speak("Good morning  pranay. Now the time is around {0} am".format(hour))
    elif hour<18:
        speak("Good afternoon pranay. Now the time is around {0} pm".format(hour))
    elif hour<21 :
        speak("Good evening pranay. Now the time is around {0} pm ".format(hour))
    else:
        speak("Good night pranay. Now the time is around {0} pm , you better go to sleep .".format(hour))
...
def mainif():
            ...
            if "introduce" in query:
                if ("introduce" and "your" and "self") :
                    speak(f" Helloo ... , I am {myName} , speed 2 giggahertz , memory half terabyte , I was created by mamulla pranay , I am here to help him")
                    mainif()
            ...
            if ("join" in query and "class" in query) or ("open" in query and "class" in query): #KMIT
                ...
                # Locate the username field by its placeholder text
                ele_user = driver.find_element(By.XPATH, "//input[@placeholder='Username']")
                ele_user.send_keys("bt21cse079@iiitn.ac.in")

# Locate the password field by its placeholder text
                ele_pass = driver.find_element(By.XPATH, "//input[@placeholder='Password']")
                ele_pass.send_keys("Pranay@360")                
                ele_login = driver.find_element(By.XPATH, "//*[@id='loginbtn']").click()
                ...
                speak("opening")
                codepath="C:\\Users\\Mamulla pranay\\AppData\\Local\\Programs\\Microsoft VS Code\\Code.exe"
                ...
                with open("C:\\Users\\Mamulla pranay\\OneDrive\\Desktop\\jarvis-main\\jarvis\\main.py","r") as f:
                    newcontent=f.read()

                with open("C:\\Users\\Mamulla pranay\\OneDrive\\Desktop\\jarvis-main\\jarvis\\destination.py","w") as f:
                    f.write(newcontent)
